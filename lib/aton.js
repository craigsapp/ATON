...
// Last Modified: Sun Jan 11 20:08:51 PST 2015
// Filename:      aton.js
...
ATON.prototype.parse = function (string) {
	return this.parseRecordArray(string.split(/\n/));
};
	...
	if (line.match(/^@{5,}|^@+\s|^@{1,4}$/)) {
		...
		if (matches = line.match(/^@@(BEGIN|START)\s*:\s*(.*)\s*$/i)) {
			...
			v.curobj = v.node.reduce(function (obj, x) {
					return (obj[x] instanceof Array) ?
							obj[x.label][x.index] : obj[x];
				}, v.output);
		...
		// \\some data line -=> \some data line
		// Only the @ or \ at the start of the line needs to be esacaped;
		// otherwise, all other @ or \ characters are literal.
...
ATON.prototype._cleanParameter = function (v) {
	if ((typeof v.curkey !== 'undefined') && v.curobj && v.curobj[v.curkey]) {
		...
		if (v.typer && (typeof v.typer[v.curkey] !== 'undefined')) {
			var newtype = v.typer[v.curkey];
...
///////////////////////////////////////////////////////////////////////////
//
// Export ATON constructor if running in v.node:
//
